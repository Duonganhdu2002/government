...
const authController = {
  ...
  register: async (req, res) => {
    try {
      const { fullname, identificationnumber, address, phonenumber, email, username, password, areacode } = req.body;
      
      // Validate that all required fields are provided
      if (!fullname || !identificationnumber || !address || !phonenumber || !email || !username || !password || !areacode) {
        return res.status(400).json({
          error: 'All fields (Full Name, Identification Number, Address, Phone Number, Email, Username, Password, and Area Code) are required.'
        });
      }
      ...
      if (userCheck.rows.length > 0) {
        return res.status(400).json({
          error: 'Username, Identification Number, Email, or Phone Number already exists.'
        });
      }
  ...
  refreshToken: async (req, res) => {
    try {
      ...
      jwt.verify(refreshToken, REFRESH_TOKEN_SECRET, async (err, decoded) => {
        ...
        const storedRefreshToken = await redisClient.get(`refreshToken_${userId}`);
